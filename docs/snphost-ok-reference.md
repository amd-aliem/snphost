# snphost ok - Test Reference Guide
...
| **SEV Firmware** | 1.51+ | Check with `snphost show` |
...
| Issue | Quick Fix |
|-------|-----------|
| Permission denied errors | Run with `sudo snphost ok` |
...
**Fix:** SEV-SNP requires AMD EPYC (Milan, Genoa, Bergamo, Siena, Turin+)
...
**Fix:** Need AMD EPYC 3rd Gen (Milan) or newer for SNP support (consumer Ryzen not supported)
...
**Fix:** Use AMD EPYC Naples or newer
...
**Fix:** Ensure AMD EPYC processor (some early SKUs may lack SEV)
...
**Fix:** Use AMD EPYC 2nd Gen (Rome) or newer
...
**Fix:** Requires AMD EPYC 3rd Gen (Milan) or newer
...
**Check (version 0 - couldn't query firmware):**
```bash
sudo snphost ok
...
**Update Firmware (version < 1.51):**

Your firmware is too old for SNP support. Update methods vary by processor generation:

| Processor | Method | Notes |
|-----------|--------|-------|
| **All EPYC** | Update BIOS/UEFI from system vendor | **Primary method** - firmware bundled with BIOS |
| **Milan (3rd Gen)** | Alternative: Runtime loading via linux-firmware | Optional, BIOS method still recommended |
| **Genoa/Turin (4th/5th Gen)** | BIOS updates only | Runtime loading **not supported** |

**Check Current Version:**
```bash
sudo snphost show | grep -i firmware
```
...
**Note:** Reboot if firmware was recently updated
...
| Issue | Cause |
|-------|-------|
| Running in VM | Must use baremetal host |
| Permission denied | Need sudo |
| Device not found | PSP driver not loaded or kernel config missing |

**Check Kernel Configuration:**
```bash
# Verify kernel has PSP and SEV support
grep -E "CONFIG_CRYPTO_DEV_CCP|CONFIG_KVM_AMD_SEV" /boot/config-$(uname -r)

# Should show:
# CONFIG_CRYPTO_DEV_CCP=y (or =m)  <- PSP/CCP driver
# CONFIG_KVM_AMD_SEV=y
```

> **Kernel Requirements:**
> - `CONFIG_CRYPTO_DEV_CCP=y` or `=m` (driver for AMD PSP - Platform Security Processor)
> - `CONFIG_KVM_AMD_SEV=y` (SEV support)
> - If missing, kernel recompile or distro upgrade needed
>
> **What is CCP?** Crypto Co-Processor driver - provides access to AMD's PSP which handles SEV/SNP operations
...
**SELinux/AppArmor:** Verify security policies aren't blocking access
