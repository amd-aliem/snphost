...
fn render_short(results: &[TestResultNode], sw_versions: &[SoftwareVersion]) {
    ...
    let sw_failures: Vec<&SoftwareVersion> = sw_versions
        .iter()
        .filter(|v| v.status == SwVersionStatus::TooOld || v.status == SwVersionStatus::PermissionDenied)
        .collect();
    if !failures.is_empty() || !sw_failures.is_empty() {
        println!("\nFAILURES:");
        for f in &failures {
        ...
        for v in &sw_failures {
            ...
            let detail = match v.status {
                SwVersionStatus::PermissionDenied => format!("Permission denied (min: {})", min),
                _ => format!("{} (min: {})", ver, min),
            };
...
fn to_json_test(node: &TestResultNode) -> JsonTestResult {
    ...
    JsonTestResult {
        ...
        .to_string(),
        label: Some(node.category.to_string()),
